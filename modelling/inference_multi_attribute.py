...
"""
Calcolo inferenza e valutazione modello di generazione di sequenze di testo con testa multipla (= più decoder/experts).
La valutazione viene effettuata su un set di dati di test e i risultati vengono registrati
in un file di ouput (f_out).
"""
...
# Definizione classi che serviranno per rappresentare l'output del modello
...
class BartModelCombined(GenerationMixinCustomCombined, nn.Module):
    def __init__(self, model1, model2, config: BartConfig):
        ...
        # due modelli distinti ognuno associato a una diversa testa
    ...
    def forward(
        ...
        """
        --> Combinazione pesata degli output delle due teste.
        A seguito del calcolo delle softmax, vi è una combinazione
        dei due modelli in modo ponderato attraverso il parametro gate_prob.
        OSS: gate_prob è qui fissato: gate_prob=0.5
        I risultati ponderati (da appunto gate_prob) vengono poi sommati 
        per ottenere lm_logits che rappresenta i logaritmi delle probabilità finali. 
        Infine, viene creato un oggetto Seq2SeqLMOutput con i valori ottenuti
        da quanto fatto precedentemente.
        """
    ...
    """
    Preparazione input del modello per generazione.
    Si pone attenzione anche alla gestione dei valori passati/precedenti,
    ossia presumibilmente ai token generati in precedenza
    """
    # unchanged
    def prepare_inputs_for_generation(
        ...
        # ossia gestione specifica durante la generazione
...
"""
Caricamento di un modello con la propria configurazione.
Restituisce il modello, i parametri e la configurazione
"""
def load_model(path):
...
"""
Valutazione modello su dataset di valutazione
"""
def evaluate(args, eval_dataset, model: PreTrainedModel, args1, args2, tokenizer: PreTrainedTokenizer,
    ...
    # Setting batch size di valutazione
    ...
    # se true -> generazione di testo nel file di output nel quale vengono appunto scritti
    # i risultati della generazione del testo
    if args.generate:
        f_out = open(os.path.join(eval_output_dir, 'test_out%s.txt' % suffix), 'w')
        print(eval_output_dir)
        k = 0

        # ingresso fase di valutazione effettiva su ongi batch
        with torch.no_grad():
            model.eval()
            ...
            for batch in eval_dataloader:
                ...
                # per ciascun elemento nel batch si decodifica il testo dai tensori usando un tokenizer
                for j in range(input_ids.shape[0]):
                    ...
                    # 'gold'  è testo/riassunto di riferimento
                    # 'gen'   è il risultato della generazione di testo
...
def main():
    ...
    """
    Caricamento dei due modelli, ossia le due teste/decoder/experts.
    Ciascuno con le rispettive configurazioni
    """
    # Set seed
    ...
    json.dump(args1, f_out) # scrive args1 in f_out
    f_out.write('\n')
