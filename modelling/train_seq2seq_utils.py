...
"""
Tale file contiene diverse funzioni utilizzate per la preparazione dei dati 
e l'adattamento del modello durante l'addestramento.
Vi sono funzioni per caricare, convertire e organizzare i dati in modo adatto e corretto
per l'addestramento de modello.
"""
...
# conversione di dati in modo che siano utilizzabili come input per il modello
# i risultati che restituisce tale funzione sono restituiti come una lista di oggetti
# che contengono le informazioni necessarie per l'input del modello
def convert_examples_to_features(examples, tokenizer, max_length=512, max_decoder_length=128):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # l'input viene tokenizzato e viene aggiunto uno spazio
        input_ids = tokenizer.encode(input, add_prefix_space=True)

        # troncamento eventuale
        if len(input_ids) > max_length:
            input_ids = input_ids[:max_length - 1]

        # calcolo del padding per raggiungere la lunghezza massima
        ...
        # output tokenizzato come fatto per input
        # se presente la supervisione sulla porta
        # output suddiviso in frasi e ogni frase viene tokenizzata separatamente
        ...
        # gli id degli input e degli output tokenizzati ma anche le maschere di attenzione
        # vengono utilizzati per creare un oggetto
...
def load_and_cache_examples(args, tokenizer, split):
    ...
    # creazione percorso del file in cache
    ...
    # i tensori vengono utilizzati per creare un oggetto TensorDataset che è un insieme di dati
    # che può essere utilizzato per l'addestramento e la valutazione
