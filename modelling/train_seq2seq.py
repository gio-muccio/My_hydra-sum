...
"""
File contenente il nucleo dell'addestramento del modello Hydrasum.
"""
...
"""
MODEL_CLASSES è un dizionario che mappa stringhe (nomi tipo-modello) a tuple di 3 elementi (3 diverse classi)
1° elemento della tupla: classe di configurazione per modello BART
2° elemento della tupla: classe personalizzata per configurazioni del modello BART specifico
3° elemento della tupla: classe del tokenizer associato al modello.

Quindi si ha una mappatura tra nomi dei tipi di modelli e classi associate a quelle configurazioni specifiche,
che verranno istanziate successivamente.
"""

# per riproducibilità risultati di addestramento
def set_seed(args):
...
# calcolo accuratezza tra previsioni, cioè i riassunti generati automaticamente dal modello,
# e i veri risultati 'gold', cioè i riassunti di riferimento/reali
# si ignora l'id -1
#   'gold' = riassunto di riferimento / "reale"
#   'pred' o 'gen' = riassunto generato automaticamente dal modello
# accuratezza = previsioni corrette diviso numero totale di esempi
...
# salvataggio checkpoint per evenutale controllo e riproducibilità
...
def evaluate(args, eval_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, prefix="") -> Dict:
    ...
    # creazione data loader (componente pytorch) per caricamento e gestione dati
    args.eval_batch_size = args.per_gpu_eval_batch_size * max(1, args.n_gpu)
    eval_sampler = SequentialSampler(eval_dataset) # campionatore: esempi in ordine sequenziale
    eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.eval_batch_size)

    softmax_function = nn.Softmax(dim=-1)
    # il calcolo del softmax avviene sull'ultima dimensione dell'input che sarà passato a softmax_function
    ...
    # se abilitato il salvataggio delle probabilità predette, si crea f_out
    if args.dump_posteriors:
        f_out = open(os.path.join(eval_output_dir, 'prob_out%s.txt' % prefix), 'w')

    # ingresso modalità di valutazione
    with torch.no_grad():
        # tutte le operazioni all'interno di questo blocco
        # non contribuiranno al calcolo dei gradienti
        # uso tipico nel momento della valutazione
        model.eval()

        preds = None
        decoder_ids_all = None

        # iterazione su dati di valutazione con visualizzazione barra di avanzamento
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            batch = tuple(t.to(args.device) for t in batch)

            # si estraggono i tensori di input, attenzione, decoder, attenzione del decoder e gate
            ...
            # Se modello a 2 o 3 decoder/teste/experts
            ...
            # Se supervisione del gate relativo alla separazione dello stile imposto (guided) o meno (unguided)
            if args.use_gate_supervision:
                outputs, _, gate_prob = model(**inputs)

                # le previsioni sono aggiornate in base al gate
                if preds is None:
            ...
            # se abilitato il salvataggio delle probabilità predette, si scrivono i risultati nel file f_out
            if args.dump_posteriors:
                logits = outputs[1]
    ...
    # valutazione se supervisione del gate
    if args.use_gate_supervision:
        # adattamento / rimodellamento dei tensori in modo che possa esser poi calcolata l'accuratezza
        preds = preds.reshape(-1, 2)
        out_label_ids_sent = out_label_ids_sent.reshape(-1)
        acc = compute_accuracy_score(y_true=out_label_ids_sent, y_pred=preds)
        print(acc)

    eval_loss = eval_loss / eval_steps # perdita/loss media
    ppl = math.exp(eval_loss) # perplessità

    # generazione di sequenze di testo effettiva
    # in f_out si scrivono i risultati della generazione
    if args.generate:
        ...
        with torch.no_grad():
            ...
            for batch in eval_dataloader:
                ...
                # generazione testo con metodo  model.generate  in base all'input fornito
    ...
    # Scrittura risultati valutazione su file
    ...
    with open(output_eval_file, "a") as writer:
        ...
        writer.write('\n')
    return result
# i risultati vengono restituiti come un dizionario contentente loss e perplessità
...
def train(args, train_dataset, eval_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer):
    ...
    # campionamento casuale dei dati
    train_sampler = RandomSampler(train_dataset)
    # creazione data loader sulla base del campionatore casuale e della dimensione del batch
    train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.train_batch_size)

    # se specificato args.max_steps
    if args.max_steps > 0:
    ...
    # addestramento su più GPU se disponibili
    if args.n_gpu > 1:
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # se si raggiunge il numero di step
        ...
        # condizione di stop di addestramento
        # o numero massimo di passaggi raggiunto
        # o numero massimo di epoche raggiunto
        if 0 < args.max_steps < global_step:
